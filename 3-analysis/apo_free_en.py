...
for i, res in enumerate(resi):
	f=os.path.join("/Scr/arango/ergosterol-amb/dgx_runs/compound_screen/analysis/", "top1."+str(res)+".*.csv")
	files = glob.glob(f)	
	df1 = pd.concat(map(pd.read_csv, files), ignore_index=True).iloc[1:,:] #[times, 2]: col0->time & col1->dihed
	#print(df1)	
	f2=os.path.join("/Scr/arango/ergosterol-amb/dgx_runs/compound_screen/analysis/", "top2."+str(res)+".*.csv")
	files2 = glob.glob(f2)
