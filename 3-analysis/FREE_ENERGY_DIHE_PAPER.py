...
resi = ["CHL.AMB", "CHL.243","CHL.AMB", "CHL.243","CHL.AMB", "CHL.243","CHL.AMB", "CHL.243"]
fig, ax = plt.subplots(2,4,sharex=True,sharey=True,constrained_layout=True,)
for i, res in enumerate(resi):
	f=os.path.join("/Scr/arango/ergosterol-amb/1-WorkflowDevelopment/3-analysis/", "out1."+str(res)+".*.csv")
	files = glob.glob(f)	
	df1 = pd.concat(map(pd.read_csv, files), ignore_index=True).iloc[1:,:] #[times, 2]: col0->time & col1->dihed
	#print(df1)	
	f2=os.path.join("/Scr/arango/ergosterol-amb/1-WorkflowDevelopment/3-analysis/", "out2."+str(res)+".*.csv")
	files2 = glob.glob(f2)
	...
	print("df2")
	print(len(df2))
	lend = abs(len(df1)-len(df2))
	df2 = df2.drop(range(len(df1), len(df2)))
	print("df2")
	print(len(df2))
	df1 = df1[:-200]
	df2 = df2[:-200]
